  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await api.post(
        '/prd/generate',
        {
          question: `Generate a PRD for: ${selected.title}${selected.feature
       ...
       *
       * NOTE: problem_statement is intentionally NOT included as its own
       * section here — it's already assigned to `overview` below, which is
       * rendered separately (both in the detail view and in
       * toMarkdown()'s "## Problem Statement" block). Including it again
       * as a section used to duplicate the same paragraph on screen.
       */
                ...
                {drafts.length > 0 ? (
                  drafts.map((draft) => (
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-lg transition-all border ${selectedId ===
                          draft._id
