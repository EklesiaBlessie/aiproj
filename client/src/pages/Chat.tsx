...
export default function Chat() {
  ...
  /**
   * Current conversation's session id, returned by the AI service on the
   * first successful reply and sent back on every subsequent message in
   * this chat so the AI service's ContextManager can see prior turns.
   *
   * Deliberately kept in React state only (not localStorage) — reloading
   * the page starts a fresh conversation, which matches "Chat" being reset
   * on navigation/refresh rather than silently resuming an old session.
   */
  ...
  const send = async (text: string) => {
    ...
    try {
       ...
       *
       * POST http://127.0.0.1:8000/copilot
       *
       * The CopilotService then decides:
       *
       * Casual conversation -> chitchat reply
       * RICE / ICE / MoSCoW -> PrioritizationService
       * PRD -> PRDService
       * Normal product question -> AIService / RAG
       *
       * `sessionId` is passed so this chat keeps its conversation history
       * across turns instead of starting fresh on every message.
       */

      const result = await requestCopilot(question, sessionId);

      // Keep using whatever session id we already have if this call somehow
      // didn't return one (e.g. the "unavailable" path) — only update it
      // when the AI service actually gave us a value.
      if (result.sessionId) {
      ...
      if (result.live) {
        ...
        if (typeof result.answer === 'string') {
          ...
          const copilotAnswer = result.data['answer'] as any;
          const intent = result.data['intent'];

          /*
           * NORMAL STRING ANSWER
           *
           * Covers both the "analyze" (RAG) intent AND the new "chitchat"
           * intent — both return a plain string in `answer`.
           */
